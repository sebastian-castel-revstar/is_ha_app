...
from models import APIRequest, APIResponse, ErrorMessage
from queryclassifierAPI import lambda_handler
...
async def reroute_path(request: Request, call_next):
    # re-route requests from /query-classifier/suffix to /suffix
    # ALB listener rules preserve full path when forwarding to ECS, but we only want /ping not /query-classifier/ping
...
@app_router.post(
    "/",
    description="Classify query intent and toxicity",
    response_model=APIResponse,
    response_description="Query classification",
    responses={
...
async def handler(event: APIRequest):
    try:
        ...
        if "INTENT_SEARCH_API_ERROR" in error_detail:
            status_code = int(error_detail.replace("INTENT_SEARCH_API_ERROR_", ""))
            return JSONResponse(
